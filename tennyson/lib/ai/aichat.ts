...
const executeToolCall = <R>(call: ToolCall, tools: Tool2<any, any, R>[]) =>
  Effect.gen(function* () {
    ...
    try {
      ...
      return result.pipe(
        Either.match({
          ...
          onRight: (data) => {
            const dataStr = Schema.encodeSync(Schema.parseJson(tool.outSchema))(
              data,
            );
            return {
              _tag: "data" as const,
...
function mkResponseTool<R>(returnSchema: Schema.Schema<R>) {
  ...
  const respondToUserTool: Tool2<R, Schema.Schema.Type<typeof outSchema>, R> = {
    tag: "type2",
    name: responseName,
    inputSchema: returnSchema,
    outSchema: outSchema,
    description:
      "Does no further tool calls or execution steps, and returns the response to the user",
    callback: async (results: R) =>
      Either.left({
        _tag: "return",
        results,
      }),
...
namespace PromptSection {
  ...
  export function fromPrompt(p: Prompt) {
    const section = (header: string, contents: string) =>
      c.id({ header, contents });

    const tools = (p.tools ?? []).map((tool: Tool2<any, any, any>, i: number) =>
      section(
    ...
    const attachments = (p?.attachments ?? []).map(
...
function finalizePromptOfPlanAndCallResults<R>(q: Query<R>) {
  const role =
...
function toolCallPromptOfQuery(q: Query<any>, prev: number, remaining: number) {
  const role = [
...
const augmentWithControlFlow = <R>(q: Query<R>) =>
  c.id({
    ...q,
    tools: (q.tools ?? []).concat(mkResponseTool(q.responseSchema)),
  });

const augmentToDemandReturn = <R>(q: Query<R>) =>
  c.id({
    ...q,
        ...
        Effect.succeed([
          {
            ...
            error: "Unexpected Tool Error",
            message:
              "The tool request JSON could not be parsed. Correct format is { toolName: <name>, data: <data> }",
            data: {
      ...
      onSome: (toolCall) =>
        Effect.gen(function* () {
          const toolResults = yield* Effect.all(
            [toolCall].map((call) => executeToolCall(call, q.tools ?? [])),
          );
...
export const query = <R>(q_: Query<R>): Effect.Effect<R, UnknownException> =>
  Effect.gen(function* () {
    ...
    const parse = yield* Schema.decode(Schema.parseJson(ToolCall))(res).pipe(
      Effect.mapError((e) => new UnknownException(e)),
    );
    const data = parse.data;
    return yield* Schema.decodeOption(Schema.parseJson(q.responseSchema))(
      data,
