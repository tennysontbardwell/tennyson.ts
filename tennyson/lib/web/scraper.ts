...
export class CacheLess implements Cache {
  ...
  async getall(version: number, name: string, params: string[]) {
    return params.map((params) => {
      return { params, results: undefined };
    });
  }
}
...
export class DBCache implements Cache {
  ...
  private dbget<T>(sql: string, sqlParams: any[] = []): Promise<T> {
    return new Promise((resolve, reject) => {
  ...
  async getall(
    ...
    return await pipe.Pipe.ofArray(params)
      ...
      .map(async (params) => {
        const placeholders = params.map(() => "?").join(",");
        const cachedParams = await this.dball<{
          params: string;
          results: string;
        }>(
          `
        ...
        const cachedMap = new Map(
          cachedParams.map((row) => [row.params, row.results]),
        );
...
export abstract class Node<P, R> {
  ...
  serializeResult(results: R): string {
    return stableStringify(results)!;
  }
  deserializeResult(results: string): R {
  ...
  async cacheall(params: P[]): Promise<void> {
    ...
    const checkResults = await this.cache.check(
      this.version,
      this.name,
      serializedParams,
    );
  ...
  async getall(params: P[]): Promise<{ params: P; results: R }[]> {
    const serializedParams = params.map(this.serializeParams);
    const cachedResults = await this.cache.getall(
      this.version,
      this.name,
      serializedParams,
    );
...
> {
  ...
  customFetcher:
    | null
    | ((url: string) => Promise<{ content: string; status: number }>) = null;
